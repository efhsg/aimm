...
class MacroIndicatorQueryTest extends Unit
{
    public function testFindByKeyAndDateReturnsArray()
    {
        $expected = ['id' => 1];
        ...
        $query = new MacroIndicatorQuery($db);
        $this->assertEquals($expected, $query->findByKeyAndDate('rig_count', $date));
    }

    public function testFindLatestByKeyReturnsArray()
    {
        $expected = ['id' => 1];
        ...
        $query = new MacroIndicatorQuery($db);
        $this->assertEquals($expected, $query->findLatestByKey('rig_count'));
    }
