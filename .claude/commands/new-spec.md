---
allowed-tools: Read, Write, Glob, Grep, AskUserQuestion, Bash(git rev-parse --show-toplevel), Bash(realpath:*), Bash(test:*), Bash(mkdir:*)
...
Never overwrite an existing `.ai/features/{name}/spec.md`. After authors fill
the visible gaps, continue with `/validate-spec` and then semantic review.
